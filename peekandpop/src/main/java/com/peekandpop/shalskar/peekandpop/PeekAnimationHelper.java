 ...
 *
 * <p>Helper class for animating the PeekAndPop views
 */
public class PeekAnimationHelper {
   ...
   *
   * <p>Animates the peek view to fade in and scale to it's full size. Also fades the peek
   * background layout in.
   */
   ...
   * Occurs on touch up.
   *
   * <p>Animates the peek view to return to it's original position and shrink. Also animate the peek
   * background layout to fade out.
   */
   ...
   *
   * <p>Animate the peek view back to it's original position and shrink it.
   */
   ...
   *
   * <p>Animate the peek view to expand slightly.
   */
   ...
   * Occurs when the peek view is flung.
   *
   * <p>Animate the peek view up towards the top of the screen. The duration of the animation is the
   * same as the pop animate, minus the time since the pop occurred.
   */
  public void animateFling(
      float velocityX, float velocityY, int duration, long popTime, float flingVelocityMax) {
    long timeDifference = System.currentTimeMillis() - popTime;
    if (context.getResources().getConfiguration().orientation
        == Configuration.ORIENTATION_PORTRAIT) {
      float translationAmount = Math.max(velocityY / 8, flingVelocityMax);
      ObjectAnimator animatorTranslateY =
          ObjectAnimator.ofFloat(peekView, "translationY", translationAmount);
      ...
      ObjectAnimator animatorTranslateX =
          ObjectAnimator.ofFloat(peekView, "translationX", translationAmount);
