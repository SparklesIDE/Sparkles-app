...
public class ViewUtils {
  ...
  public static Rect[] getWindowAndViewRects(View view, int statusBarHeight) {
    ...
    windowAvailableRect =
        new Rect(
            windowRect.left, windowRect.top + actionBarHeight, windowRect.right, windowRect.bottom);
    ...
    if (isInMultiWindowMode) {
      if (displayOrientation == Configuration.ORIENTATION_PORTRAIT) {
        ...
        if (statusBarHeight != windowRect.top) {
          if (view_utils_logging_enabled) viewTop += windowRect.top;
        } else {
        }
    ...
    if (displayOrientation == Configuration.ORIENTATION_LANDSCAPE
      ...
      if (view_utils_logging_enabled) windowAvailableRect.right = viewRight;
    }
   ...
   *     otherwise android will throw: `java.lang.IllegalArgumentException: Used non-visual Context
   *     to obtain an instance of WindowManager. Please use an Activity or a ContextWrapper around
   *     one instead.`
